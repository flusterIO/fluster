...
<SomeComponent 
    ...
    // It is valid typescript, but not part of the ReactNode type.
    // While externally developed plugins are free to build for 
    // their own use case, internally developed components will never 
    // use this type in user accessible components.
