...
use crate::{
    ...
    features::{
        flashcard::data::models::flashcard_tag_model::FlashcardTagModel,
        search::types::PaginationProps,
    },
};

pub struct FlashcardTagEntity {}

type T = FlashcardTagModel;

impl FlashcardTagEntity {
    ...
    pub async fn get_by_flashcard_ids(
        ...
        let items_batch = tbl
            ...
            .await
            .map_err(|e| {
                println!("Error in FlashcardTagEntity.get_by_file_paths: {:?}", e);
    ...
    pub async fn create_many(db: &FlusterDb<'_>, items: Vec<T>) -> FlusterResult<()> {
        let all_flashcard_tags = FlashcardTagEntity::get_all(
            db,
            PaginationProps {
                per_page: 9999999,
                page_number: 1,
        ...
        let filtered_tags: Vec<&T> = items
            ...
            .filter(|x| {
                !all_flashcard_tags
                    .iter()
                    .any(|y| (x.flashcard_id == y.flashcard_id) && (x.tag_value == y.tag_value))
            })
            .collect();
        let schema = FlashcardTagEntity::arrow_schema();
        let tbl = get_table(db, FlashcardTagEntity::table()).await?;
        let batches: Vec<Result<RecordBatch, ArrowError>> = filtered_tags
            .iter()
            .map(|x| Ok(FlashcardTagEntity::to_record_batch(x, schema.clone())))
            .collect();
...
impl DbEntity<T> for FlashcardTagEntity {
    ...
    fn to_record_batch(
        ...
        RecordBatch::try_new(schema, vec![Arc::new(flashcard_id), Arc::new(tag_value)]).unwrap()
    }
}
