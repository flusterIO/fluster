...
use crate::{
    ...
    features::{
        flashcard::data::models::flashcard_topic_model::FlashcardTopicModel,
        search::types::PaginationProps,
    },
};

pub struct FlashcardTopicEntity {}

type T = FlashcardTopicModel;

impl FlashcardTopicEntity {
    fn table() -> DatabaseTables {
        DatabaseTables::FlashcardTopic
    }
    ...
    pub async fn get_by_values(
        ...
        let tbl = get_table(db, DatabaseTables::FlashcardTopic).await?;
        ...
        let items_batch = tbl
            ...
            .await
            .map_err(|e| {
                println!("Error in FlashcardTopicEntity.get_by_values: {:?}", e);
    ...
    pub async fn get_by_flashcard_ids(
        ...
        let items_batch = tbl
            ...
            .await
            .map_err(|e| {
                println!("Error in FlashcardTopicEntity.get_by_file_paths: {:?}", e);
                FlusterError::FailToConnect
            })?
            .try_collect::<Vec<_>>()
            .await
    ...
    pub async fn create_many(db: &FlusterDb<'_>, items: Vec<T>) -> FlusterResult<()> {
        let all_note_tags = FlashcardTopicEntity::get_all(
            db,
            PaginationProps {
                per_page: 9999999,
                page_number: 1,
        ...
        let batches: Vec<Result<RecordBatch, ArrowError>> = filtered_tags
            .iter()
            .map(|x| Ok(FlashcardTopicEntity::to_record_batch(x, schema.clone())))
            .collect();
...
impl DbEntity<T> for FlashcardTopicEntity {
    ...
    fn to_record_batch(
        ...
        RecordBatch::try_new(schema, vec![Arc::new(flashcard_id), Arc::new(topic_value)]).unwrap()
    }
}
