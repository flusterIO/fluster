...
impl KanbanBoardListEntity {
    ...
    pub async fn get_many(
    ...
    ) -> FlusterResult<Vec<KanbanBoardListModel>> {
        let tbl = get_table(db, DatabaseTables::KanbanBoardList).await?;
        ...
        let items_batch = query
            ...
            .await
            .map_err(|e| {
                println!("Error in KanbanBoardListEntity.get_many: {:?}", e);
        ...
        let mut items: Vec<KanbanBoardListModel> = Vec::new();

        for batch in items_batch.iter() {
            let data: Vec<KanbanBoardListModel> = from_record_batch(batch).map_err(|e| {
                println!("Error: {:?}", e);
    ...
    pub async fn save_many(
        db: &FlusterDb<'_>,
        entries: &[KanbanBoardListModel],
    ) -> FlusterResult<()> {
        let schema = KanbanBoardListEntity::arrow_schema(None);
        let tbl = get_table(db, DatabaseTables::KanbanBoardList).await?;
        let batches: Vec<Result<RecordBatch, ArrowError>> = entries
            .iter()
            .map(|x| Ok(KanbanBoardListEntity::to_record_batch(x, schema.clone())))
            .collect();
...
impl DbEntity<KanbanBoardListModel> for KanbanBoardListEntity {
    ...
    fn to_record_batch(
        ...
        let board_id = StringArray::from(vec![item.board_id.clone()]);
