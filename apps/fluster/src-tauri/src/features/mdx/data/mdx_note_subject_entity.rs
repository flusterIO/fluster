...
use super::mdx_note_subject_model::MdxNoteSubjectModel;

pub struct MdxNoteSubjectEntity {}

type T = MdxNoteSubjectModel;

impl MdxNoteSubjectEntity {
    ...
    pub async fn get_by_file_paths(
        ...
        let items_batch = tbl
            ...
            .await
            .map_err(|e| {
                println!("Error in MdxNoteSubjectEntity.get_by_file_paths: {:?}", e);
                FlusterError::FailToConnect
            })?
            .try_collect::<Vec<_>>()
            .await
    ...
    pub async fn create_many(db: &FlusterDb<'_>, items: Vec<T>) -> FlusterResult<()> {
        let all_note_tags = MdxNoteSubjectEntity::get_all(
            db,
            PaginationProps {
                per_page: 9999999,
                page_number: 1,
        ...
        let batches: Vec<Result<RecordBatch, ArrowError>> = filtered_tags
            .iter()
            .map(|x| Ok(MdxNoteSubjectEntity::to_record_batch(x, schema.clone())))
            .collect();
...
impl DbEntity<T> for MdxNoteSubjectEntity {
    ...
    fn to_record_batch(
        ...
        RecordBatch::try_new(
            schema,
            vec![Arc::new(mdx_note_file_path), Arc::new(subject_value)],
        )
        .unwrap()
    }
}
