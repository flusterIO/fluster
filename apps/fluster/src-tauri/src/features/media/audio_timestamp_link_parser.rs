...
pub struct AudioTimestampLinkParser {}

impl AudioTimestampLinkParser {
    pub fn get_regex(&self) -> Regex {
        Regex::new(r#"\[(?<body>[^\]]*)\]\(audio:(?<video_id>[^@]*)@(?<timesamp>[^\)]*)\)"#)
            .expect("Creates audio timestamp link without error.")
    }
}

impl MdxParser<String> for AudioTimestampLinkParser {
    fn parse_mdx(&self, content: &str) -> ParsedContentResult<String> {
        ...
        while let Some(regex_match) = regex.captures(content) {
            ...
            new_content = new_content.replace(
                match_content,
                &format!(
                    r#"<AudioTimestampLink id='{}' timestamp="{}">{}</AudioTimestampLink>"#,
