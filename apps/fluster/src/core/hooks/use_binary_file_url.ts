...
export const useBinaryFileUrl = (filePath: string) => {
    const basePath = useSelector((appState: AppState) => appState.core.notesDirectory);
    ...
    const getData = async (fp: string, bp: string): Promise<void> => {
        ...
        if (res.status === "ok") {
            ...
            showToast({
                ...
                body: res.error === "FileDoesNotExist" ? `The file at ${filePath} does not exist.` : `Fluster ran into an error while attempting to load the file at ${filePath}`,
                duration: 5000,
                variant: "Error"
            })
        }
