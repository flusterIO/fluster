...
async loadBinaryFile(rootRelativePath: string, basePath: string) : Promise<Result<number[], FlusterError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("load_binary_file", { rootRelativePath, basePath }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async getParsableFiles(opts: GetParsableFilesOptions) : Promise<Result<ParsableFiles, FlusterError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_parsable_files", { opts }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async backupDatabaseObjects(outputDir: string) : Promise<Result<null, FlusterError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("backup_database_objects", { outputDir }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async getRecentlyAccessedNotes() : Promise<Result<MdxNoteGroup[], FlusterError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_recently_accessed_notes") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async getAllEmbeddedDocs() : Promise<Result<EmbbeddedDocFile[], FlusterError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_all_embedded_docs") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async getEquationById(id: string) : Promise<Result<EquationData, FlusterError>> {
    try {
 ...
 * date.
 */
async addAiChatRequest(chatId: string, ai: AiSyncSettings, chatInput: AiChatMessageModel, chatHistory: AiChatMessageModel[], streamChannel: TAURI_CHANNEL<AiChatMessageUpdateEventProps>) : Promise<Result<AiChatMessageModel, FlusterError>> {
    try {
...
async createNewKanbanBoardCard(item: KanbanCardModel) : Promise<Result<null, FlusterError>> {
    try {
 ...
 */
export type AiChatModel = { id: string; label: string; model: string; temperature: number; repeat_penalty: number; top_k: number; top_p: number; ctime: string }
...
export type EquationData = { equation: EquationModel; tags: SharedTaggableModel[] }
 ...
 * user's notes.
 */
export type FrontMatterBaseModel = { id: string; mdx_note_file_path: string; 
/**
 * The user_provided_id is the id field as it appears in the user's note.  The id field in
 * this struct is generated by Surreal.
 */
user_provided_id: string | null; title: string; summary: string | null; list_id: string | null; list_index: string | null; subject: string | null; topic: string | null }
/**
 ...
 * How to contribute
 */
"HowToContribute" | "GettingStarted" | "ColorProps" | "IntroToJsx" | "JupyterSetup" | "QuickReference" | "Mermaid" | "OllamaSetup"
...
export type KanbanCardModel = { id: string; label: string; desc: string | null; body: string | null; 
/**
