...
export const AiChatContainer = (): ReactNode => {
    ...
    const handleSubmitMessageRequest = async (): Promise<void> => {
        const chatId = searchParams.get("chat_id");
        if (chatId && inputValue.current.length) {
            dispatch({
                type: "setLoadingState",
                payload: AiLoadingState.pending,
            });
            const res = await commands.addAiChatRequest(chatId, inputValue.current);
            console.log("res: ", res);
            dispatch({
