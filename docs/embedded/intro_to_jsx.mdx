...
**JSX** is an extension of javascript that's used by React as well as several other single page application frameworks. **TSX** is _almost_ identical, except **TSX** adds the ability to more strictly define types. For the purposes of using Fluster, **TSX** and **JSX** can be referenced interchangeably. 
...
Through the great work at the people over at [MDX](https://mdxjs.com), we can now embed entire React components directly in markdown files, with the `.mdx` file extension. All of the original functionality of markdown remains intact, and in Fluster is actually extended quite a bit, but users can now include neatly bundled **HTML**, **CSS**, and **Javascript** in pre-defined React components. 

> All you need to know is how to properly pass in the data that these components need to calculate what they.
...
> **Note**: The `\` character signifies an 'escape character' in most programming languages, including inside of Fluster. For this reason, it is sometimes required to use 2 `\` characters when writing math inside of a string, to give `title="my title $\\delta$$"{:mdx}`.
...
<Admonition title="Children String Properties" type="info" foldable>

   **MDX** and other markdown to html parsers wrap new lines, including the children property in a paragraph element. This can cause issues in cases where the `children` component is specifically of a type string. Fluster attempts to parse these paragraphs dynamically, but in cases of a parsing error where a property of type `string` is critical like in a **JupyterCell** component, you can wrap the `children` in a pair of curly braces with a pair of backticks inside. 
...
<Hint note>
    ...
    Follow [this blog post](/blog/ulldPreBetaLaunch) to stay up to day on features coming to Fluster.
</Hint>
...
An array, if you're unfamiliar, from a high level is just another name for a list of data. Arrays in Typescript, which is the syntax used by the **x** in mdx, are denoted by surrounding a list of values with `[]` brackets, and seperating the items by commas. 

In the `PieChart` component for example, instead of providing a single `x` and `y` field, it accepts an array of objects:
...
Note, however, that the array must be wrapped in `{}` braces the same way a boolean field is wrapped in these braces. While optional for some data types, if you're unsure, just surround the value with curly braces. The only downside is an additional couple bytes of data.
...
Fluster takes advantage of this structure throughout it's **MDX** interface, but two instances are currently the most common. 
...
If you notice a bug with the way **HTML** attributes are handled, please submit an issue on Github.
