...
# Write a very simple bin package in temp folder that depends on build_tool_renamed package
# from Cargokit. This is done to ensure that we don't pollute Cargokit folder
# with .dart_tool contents.

BUILD_TOOL_PKG_DIR="$BASEDIR/build_tool_renamed"
...
name: build_tool_renamed_runner
version: 1.0.0
publish_to: none
...
cat << EOF > "bin/build_tool_renamed_runner.dart"
import 'package:build_tool_renamed/build_tool_renamed.dart' as build_tool_renamed;
void main(List<String> args) {
  build_tool_renamed.runMain(args);
}
EOF
...
# is the case for our build_tool_renamed_runner. So instead we precompile the package
# ourselves.
# To invalidate the cached kernel we use the hash of ls -LR of the build_tool_renamed
# package directory. This should be good enough, as the build_tool_renamed package
...
if [ ! -f "$PACKAGE_HASH_FILE" ]; then
    ...
    "$DART" compile kernel bin/build_tool_renamed_runner.dart
    echo "$PACKAGE_HASH" > "$PACKAGE_HASH_FILE"
fi

set +e

"$DART" bin/build_tool_renamed_runner.dill "$@"
